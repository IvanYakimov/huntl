...
namespace memory {
	...
	TEST_F(HolderTest, instanceof) {
		...
		auto i32_holder = Wrapper<uint32_t>::Create(42);
		auto i16_holder = Wrapper<uint16_t>::Create(42);
		...
		ASSERT_FALSE(instanceof<Wrapper<uint32_t>>(e_holder));
		...
		ASSERT_FALSE(instanceof<Wrapper<uint32_t>>(nil_holder));

		ASSERT_TRUE(instanceof<Holder>(i32_holder));
		ASSERT_TRUE(instanceof<Wrapper<uint32_t>>(i32_holder));
		ASSERT_FALSE(instanceof<Symbolic>(i32_holder));
		ASSERT_FALSE(instanceof<Wrapper<uint16_t>>(i32_holder));
