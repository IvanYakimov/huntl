...
class TemplateHandler (xml.sax.ContentHandler):
    ...
    def printDeclaration(self, header):
         print "\t" + header + ";"
         
    def printDefinition(self, header, body):
         print "\t" + header + "{\n\t\t" + body + "\n\t}\n"
         
    def declareOrdinary(self, header, body):
         self.printDeclaration(header)
         
    def defineOrdinary(self, header, body):
         self.printDefinition(header, body)
         
    def declareTemplated(self, header, body, id_, e):
         self.printDeclaration(header.replace(id_, e))
         
    def defineTemplated(self, header, body, id_, e):
         self.printDefinition(header.replace(id_, e), body.replace(id_, e))
    ...
    def endElement(self, tag):
        if tag == "func":
            ...
            if not templateStack.isEmpty():
                if filety == kDef:
                     self.handleTemplated(self.defineTemplated)
                elif filety == kDecl:
                     self.handleTemplated(self.declareTemplated)
                else: sys.exit()
            else:
                if filety == kDef:
                     self.handleOrdinary(self.defineOrdinary)
                elif filety == kDecl:
                     self.handleOrdinary(self.declareOrdinary)
                else: sys.exit()
            self.body = ""
            self.header = ""
